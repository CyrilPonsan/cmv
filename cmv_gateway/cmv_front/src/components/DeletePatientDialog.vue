...
watch(props.patient, (newValue) => {
  console.log('patient updated:', newValue)
})
</script>
  ...
  >
    ...
    <template #footer>
      <div class="w-full flex gap-x-4 mt-12">
        <Button fluid severity="warn" @click="emit('confirm')">Confirmer</Button>
        <Button fluid severity="secondary" variant="text" @click="emit('cancel')">Annuler</Button>
      </div>
    </template>
  </Dialog>
</template>
